...
class CoffeeCapsuleHolder extends Boxes {
    ...
    render_plate(screw_hole, hole_renderer, move) {
        let width = (((this.columns * (this.lid_size_with_margin + this.column_spacing)) - this.column_spacing) + (2 * this.outer_margin));
        ...
        this.polyline((width - (2 * this.corner_radius)), [90, this.corner_radius], (height - (2 * this.corner_radius)), [90, this.corner_radius], (width - (2 * this.corner_radius)), [90, this.corner_radius], (height - (2 * this.corner_radius)), [90, this.corner_radius]);
        this.ctx.restore();
        if (hole_renderer) {
            for (let col = 0; col < this.columns; col += 1) {
                this.ctx.save();
                this.moveTo(((this.outer_margin + (col * (this.lid_size_with_margin + this.column_spacing))) - this.burn), ((this.outer_margin + ((this.rows - 0.5) * this.lid_size)) + this.burn), -90);
                hole_renderer();
                this.ctx.restore();
            }
        }
    ...
    render_front_hole() {
        ...
        let half = [0, [degrees, (this.lid_size_with_margin / 2)], 0, -degrees, (((this.rows - 1) * this.lid_size) - height_difference)];
        let path = (((half + [[180, (this.body_size / 2)]]) + list(reversed(half))) + [[180, (this.lid_size_with_margin / 2)]]);
        this.polyline(...path);
    }
...
module.exports.CoffeeCapsuleHolder = CoffeeCapsuleHolder;