...
class PaperBox extends Boxes {
    ...
    render() {
        ...
        let path = (this.design === "tuckbox" ? this.tuckbox(this.x, this.y, this.h) : this.widebox(this.x, this.y, this.h));
        this.polyline(...path);
    }

    tuckbox(width, length, height) {
        let lid_cut_length = Math.min(10, (length / 2), (width / 5));
        let half_side = ((((((((((this.mark(this.mark_length) + [0, 90]) + this.ear_description(length, lid_cut_length)) + [0, -90, length, 0]) + this.lid_cut(lid_cut_length)) + this.lid_tab((width - (2 * this.thickness)))) + [0]) + this.lid_cut(lid_cut_length)) + [length, -90]) + this.ear_description(length, lid_cut_length)) + this.mark(this.mark_length));
        return ((((((((([height, 0] + half_side) + this.side_with_finger_hole(width, this.finger_hole_diameter)) + this.mark(this.mark_length)) + [0, 90]) + this.tab_description(((length - this.margin) - this.thickness), height)) + [0, 90]) + this.mark(this.mark_length)) + [width]) + list(reversed(half_side)));
    }

    widebox(width, length, height) {
        let half_side = ((((((((((((this.mark(this.mark_length) + [0, 90]) + this.tab_description(((length / 2) - this.margin), height)) + [0, -90, height, 0]) + this.mark(this.mark_length)) + [0, 90]) + this.tab_description(this.lid_sides, length)) + [0, 90]) + this.mark(this.mark_length)) + [height, -90]) + this.tab_description(((length / 2) - this.margin), height)) + [length, 0]) + this.mark(this.mark_length));
        return (((this.side_with_finger_hole(width, this.finger_hole_diameter) + half_side) + this.lid_tab(width)) + list(reversed(half_side)));
    }

    lid_tab(width) {
        return [(this.lid_height - this.lid_radius), [90, this.lid_radius], (width - (2 * this.lid_radius)), [90, this.lid_radius], (this.lid_height - this.lid_radius)];
    }
    ...
    mark(length) {
        ...
        return [0, -90, length, 180, length, -90];
    }

    lid_cut(length, reverse) {
        let path = [90, (length + this.thickness), -180, length, 90];
        return ([0] + (reverse ? list(reversed(path)) : path));
    }

    side_with_finger_hole(width, finger_hole_diameter) {
        let half_width = ((width - finger_hole_diameter) / 2);
        return [half_width, 90, 0, [-180, (finger_hole_diameter / 2)], 0, 90, half_width, 0];
    }
    ...
    tab_description(height, width) {
        ...
        return [0, (deg - 90), side, (90 - deg), end_width, (90 - deg), side, (deg - 90)];
    }

    ear_description(length, lid_cut_length, reverse) {
        ...
        let path = [start_margin, -90, lid_cut_length, 90, 0, [-90, radius], 0, 90, (((length - radius) - start_margin) - end_margin), 90, ear_depth, -90, end_margin];
        return ((reverse ? list(reversed(path)) : path) + [0]);
    }

}

module.exports.PaperBox = PaperBox;