import { mmul, rotm, vtransl } from './vectors.js';

// Helper functions from Python version

/**
 * Calculate tooth spacing using curve fit polynomial.
 * @param {number} teeth - Number of teeth.
 * @param {number} b - Coefficient b.
 * @param {number} c - Coefficient c.
 * @param {number} d - Coefficient d.
 * @returns {number} Calculated diameter/spacing value.
 */
...
/**
 * Calculate tooth spacing based on pitch and offset.
 * @param {number} teeth - Number of teeth.
 * @param {number} tooth_pitch - Pitch of the teeth.
 * @param {number} pitch_line_offset - Pitch line offset.
 * @returns {number} Calculated diameter.
 */
...
/**
 * Mirror points across the X-axis (invert X coordinates).
 * @param {number[][]} points - List of [x, y] points.
 * @returns {number[][]} Mirrored points.
 */
...
/**
 * Class for generating timing belt pulleys.
 */
class Pulley {
    // Static data from Python version
    static spacing = {
    ...
    static teeth = {
        "MXL": [[-0.660421, -0.5], [-0.660421, 0], [-0.621898, 0.006033], [-0.587714, 0.023037], [-0.560056, 0.049424], [-0.541182, 0.083609], [-0.417357, 0.424392], [-0.398413, 0.458752], [-0.370649, 0.48514], [-0.336324, 0.502074], [-0.297744, 0.508035], [0.297744, 0.508035], [0.336268, 0.502074], [0.370452, 0.48514], [0.39811, 0.458752], [0.416983, 0.424392], [0.540808, 0.083609], [0.559752, 0.049424], [0.587516, 0.023037], [0.621841, 0.006033], [0.660421, 0], [0.660421, -0.5]],
        ...
        "HTD_8mm": [[-3.301471, -1], [-3.301471, 0], [-3.16611, 0.012093], [-3.038062, 0.047068], [-2.919646, 0.10297], [-2.813182, 0.177844], [-2.720989, 0.269734], [-2.645387, 0.376684], [-2.588694, 0.496739], [-2.553229, 0.627944], [-2.460801, 1.470025], [-2.411413, 1.691917], [-2.343887, 1.905691], [-2.259126, 2.110563], [-2.158035, 2.30575], [-2.041518, 2.490467], [-1.910478, 2.66393], [-1.76582, 2.825356], [-1.608446, 2.973961], [-1.439261, 3.10896], [-1.259169, 3.22957], [-1.069074, 3.335006], [-0.869878, 3.424485], [-0.662487, 3.497224], [-0.447804, 3.552437], [-0.226732, 3.589341], [-0.000176, 3.607153], [0.226511, 3.589461], [0.447712, 3.552654], [0.66252, 3.497516], [0.870027, 3.424833], [1.069329, 3.33539], [1.259517, 3.229973], [1.439687, 3.109367], [1.608931, 3.10896], [1.766344, 2.825731], [1.911018, 2.664271], [2.042047, 2.490765], [2.158526, 2.305998], [2.259547, 2.110755], [2.344204, 1.905821], [2.411591, 1.691983], [2.460801, 1.470025], [2.553229, 0.627944], [2.588592, 0.496739], [2.645238, 0.376684], [2.720834, 0.269734], [2.81305, 0.177844], [2.919553, 0.10297], [3.038012, 0.047068], [3.166095, 0.012093], [3.301471, 0], [3.301471, -1]],
    ...
    /**
     * Get available pulley profiles.
     * @returns {string[]} List of profile names (e.g. "GT2_2mm", "T5").
     */
    static getProfiles() {
        return Object.keys(Pulley.teeth).sort();
    }

    /**
     * Calculate pulley diameter based on number of teeth and profile.
     * @param {number} teeth - Number of teeth.
     * @param {string} profile - Profile name.
     * @returns {number} The calculated diameter.
     */
    diameter(teeth, profile) {
        if (Pulley.spacing[profile][0]) {
            return tooth_spaceing_curvefit(teeth, ...Pulley.spacing[profile].slice(1));
        }
        return tooth_spacing(teeth, ...Pulley.spacing[profile].slice(1));
    }
     ...
     * @param {number} teeth - Number of teeth.
     * @param {string} profile - Profile name.
     * @param {boolean} [insideout=false] - If true, teeth face inward (ring gear).
     * @param {number} [r_axle=null] - Axle hole radius.
     * @param {Function} [callback=null] - Callback for center features.
     * @param {string} [move=""] - Move commands.
     */
    __call__(teeth, profile, insideout = false, r_axle = null, callback = null, move = "") {
        // ********************************
        // ** Scaling tooth for good fit **
        // ********************************
        // To improve fit of belt to pulley, set the following constant. Decrease or increase by 0.1mm at a time. 
        // We are modelling the *BELT* tooth here, not the tooth on the pulley. Increasing the number will 
        // *decrease* the pulley tooth size. Increasing the tooth width will also scale proportionately the 
        // tooth depth, to maintain the shape of the tooth, and increase how far into the pulley the tooth 
        // is indented. Can be negative

        const additional_tooth_width = 0.2;  // mm

        // If you need more tooth depth than this provides, adjust the following constant. However, this will cause the shape of the tooth to change.
        const additional_tooth_depth = 0;  // mm

        let pulley_OD = this.diameter(teeth, profile);

        const [tooth_depth, tooth_width] = Pulley.profile_data[profile];
        const tooth_distance_from_centre = Math.sqrt((pulley_OD / 2) ** 2 - ((tooth_width + additional_tooth_width) / 2) ** 2);
        const tooth_width_scale = (tooth_width + additional_tooth_width) / tooth_width;
        let tooth_depth_scale = (tooth_depth + additional_tooth_depth) / tooth_depth;
        ...
        const total_width = Math.max(pulley_OD, 2 * (r_axle || 0.0));

        if (this.boxes.move(total_width, total_width, move, true)) {
            return;
        }

        this.boxes.moveTo(total_width / 2, total_width / 2);
        this.boxes.cc(callback, null, 0.0, 0.0);

        if (r_axle) {
            if (insideout) {
                this.boxes.circle(0, 0, r_axle);
            } else {
                this.boxes.hole(0, 0, r_axle);
            }
        }
        ...
        for (let i = 0; i < teeth; i++) {
            const m = [
                [tooth_width_scale, 0, 0],
                [0, tooth_depth_scale, -tooth_distance_from_centre]
            ];
            const rotationMatrix = mmul(m, rotm(i * 2 * Math.PI / teeth));

            // Transform each tooth profile point
            for (const pt of Pulley.teeth[profile].slice(1, -1)) {
                points.push(vtransl(pt, rotationMatrix));
            }
        }

        this.boxes.drawPoints(points, insideout ? -1 : 1);
        this.boxes.move(total_width, total_width, move);
    }
}
